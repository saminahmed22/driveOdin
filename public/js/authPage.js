...
const rePasswordInputContainer = document.querySelector(
  ".rePasswordInputField",
);

if (rePasswordInputContainer) {
  rePasswordInputContainer.addEventListener("click", (event) => {
    const button = event.target.closest("button");

    if (!button) return;

    const buttonImg = button.querySelector("img");
    const inputField = rePasswordInputContainer.querySelector("input");
